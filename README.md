...
devcontainer をサンドボックスとして扱うための隔離パターン集。AI エージェント (Claude Code / Cursor / Codex 等) を運用する開発者向けに、**標準的な devcontainer 構成から一歩踏み込んだ隔離要件 — 秘匿情報を作業コンテナの外に置き、許可ドメイン内でも操作粒度を絞って通す — をどう実現するか** を追求した技術的探索の成果をまとめたもの。
...
この踏み込んだ目標を 1 つの作業コンテナ内で実装しようとすると、ファイル ACL / プロセス権限 / 通信制御など複数のレイヤを同時に整える必要があり、どれか一つが崩れたら全体が崩れる構造になる。本書では、外部操作を通すかの判定をプロキシ群に集めることで、何が崩れたら全体が崩れるかの線 (= 信頼境界) を 1 箇所に揃える (詳細は [docs/02-design.md](./docs/02-design.md))。
...
1. [01-problem.md](./docs/01-problem.md) — 踏み込んだ隔離要件を明確に記述する動機
