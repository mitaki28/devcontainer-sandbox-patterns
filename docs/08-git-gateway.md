...
これらは本書の安全性モデル ([02-design.md](./02-design.md) §2) のスコープ外で、git-gateway の ref / commit ACL では塞ぎきれない。**最低限、private リポジトリ + CI 実行が無効化された環境** であることが本レシピの利用前提となる。public リポジトリ / CI 実行が有効な環境では、リポジトリ設定や CI 運用フロー側で別途防ぐ必要があり、それだけで本書と同等の規模のレシピ集が要る領域となる。
