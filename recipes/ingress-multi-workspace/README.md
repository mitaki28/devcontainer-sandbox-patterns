...
1. **作業コンテナ間の到達可能性**: shared ネットワークに全作業コンテナが参加するため、作業コンテナ A が作業コンテナ B の `:3000` に到達可能。個人開発者が並列で作業コンテナを回す前提では許容範囲とする (`integrated/multi-workspace/README.md` の同節と同じ性質)
