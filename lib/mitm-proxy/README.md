...
├── Dockerfile.workspace      # node:22-slim + ca-certificates + curl + git + bootstrap entrypoint
...
└── test/                     # smoke 専用の閉鎖環境スタック
    ...
    ├── Dockerfile.mock       #   - mock-target イメージ (node:22-slim + openssl で複数 SAN 証明書を毎回生成)
...
- Node fetch / curl: readonly_hosts に居るホスト (`api.test` / `registry.test`) への GET → 200
...
4. **ワークロードは非 root で動かす**: `bootstrap-ca.sh` は root で起動し、`update-ca-certificates` 完了直後に `setpriv` で `WORKLOAD_USER` (呼び出し側で必須明示、本 lib の `Dockerfile.workspace` と integrated/single-workspace 等の devcontainers 系ベースのいずれも `node`) に drop してから CMD を `exec` する。デフォルトを持たせず未指定で起動拒否にしているのは、ベースイメージごとに実在するユーザが異なるため、漏れに気付かず権限降格が壊れたまま動く事故を防ぐため
...
- `header_inject` … allow されたリクエストにヘッダを注入する。プライベートレジストリの Bearer など、ホストを限定して付ける用途
...
any-match-allows。`allow_rules` を `readonly_hosts` より先に評価することで「readonly ホストへの副作用のない POST」が表現できる (readonly を先にすると POST が unsafe-method で即拒否されてしまう)。原則拒否の構造下では「allow に書かない」=「拒否」で塞げるため、`deny_rules` は持たない。
...
mitmproxy のヘルスチェックは `mitmproxy-ca-cert.pem` の存在で「立ち上がって CA 生成済み」を判定する。smoke で実 fetch が通ることで listen 完了は事実上検証されている。
...
5. **ビルド時の外向き通信**: 作業コンテナのイメージビルド時の `apt-get` / `pnpm install` 等はビルド時の外向き通信を要する。本レシピでは `Dockerfile.workspace` が build context で外に出るのを許容している (`alternatives/dependencies-build-time/` のように実行時にゼロにはしていない)
