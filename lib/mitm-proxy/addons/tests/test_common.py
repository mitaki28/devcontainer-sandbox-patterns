...
検証する分岐:
- 既に response が立っている (先行 addon が deny 済) → 何もしない
...
class CommonPolicyTest(unittest.TestCase):
    ...
    def test_existing_response_not_overwritten(self):
        # 先行 addon (SniGuard / HostSanGuard) が deny して response を立てたケース。
        # CommonPolicy は response を上書きしない。
        flow = make_flow(host="example.com")
        flow.response = http.Response.make(200, b"already set upstream")
        with patched_config(
