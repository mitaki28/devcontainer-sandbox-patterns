...
allow/deny の判定は CommonPolicy が行うので、本 module は単に「まだ response が
立っていない request」に対してヘッダを足すだけ。最初に一致した rule のみ適用する。

順序: addons リストの末尾近くに置き、CommonPolicy が allow した request だけに
inject が走るようにする。CommonPolicy が deny した場合は response が立つので skip。
"""
