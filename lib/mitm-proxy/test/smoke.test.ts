...
// 3. proxy 経由 + 各種ツール (Bun fetch / curl) で HTTPS が成立する
...
describe("lib/mitm-proxy smoke (CA bootstrap + ACL + header inject + 攻撃模倣)", () => {
  test("workload は非 root (bun, uid=1000) で動いている", () => {
    // bootstrap-ca.sh が CA install 後 setpriv で bun に drop している前提。
  ...
  test("CA cert ファイルが trust store にある", () => {
    expect(existsSync("/usr/local/share/ca-certificates/mitmproxy.crt")).toBe(true);
    const indiv = readFileSync("/usr/local/share/ca-certificates/mitmproxy.crt", "utf8");
    expect(indiv).toContain("BEGIN CERTIFICATE");
    expect(indiv).toContain("END CERTIFICATE");
  });
  ...
  test("Bun fetch: api.test (readonly exact, GET) → 200", async () => {
    const res = await fetch("https://api.test/", { proxy: PROXY });
    expect(res.status).toBe(200);
    expect(await res.text()).toContain("mock-ok GET");
  });
  ...
  test("readonly_hosts 外 (denied.test) は 403 で deny", async () => {
    const res = await fetch("https://denied.test/", { proxy: PROXY });
  ...
  test("readonly_hosts の glob (`*.content.test`) で raw.content.test が allow される", async () => {
    ...
    const res = await fetch("https://raw.content.test/some/path", { proxy: PROXY });
    expect(res.status).toBe(200);
    expect(await res.text()).toContain("mock-ok GET");
  });
  ...
  test("readonly_hosts: echo.test への POST (allow_rules 不一致 path) は 403 で deny", async () => {
    ...
    expect(res.status).toBe(403);
    expect(await res.text()).toContain("readonly");
  });
  ...
  test("allow_rules: readonly host (echo.test) でも /anything/foo への POST は個別に許可されて通る", async () => {
    ...
    expect(data.url).toContain("/anything/foo");
  });
  ...
  test("HostGuard: 偽 Host ヘッダ (実宛先 denied.test, Host: api.test) は 403", () => {
    ...
    // 403 で deny する境界を担う。
    //
    // Bun の fetch は Host を URL から自動正規化してくれて override が効きにくいので
  ...
  test("HeaderInjector: policy.json の rule で echo.test/headers にヘッダ注入される", async () => {
    ...
    const res = await fetch("https://echo.test/headers", { proxy: PROXY });
    expect(res.status).toBe(200);
    ...
    expect(entry![1]).toBe("smoke-injected");
  });
